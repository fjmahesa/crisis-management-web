...
export default function BlogTest() {
  ...
  useEffect(() => {
    fetch('https://admin.crisismanagement.id/wp-json/wp/v2/posts?_embed')
      ...
      .then((data) => {
        ...
        // KUNCI: Ambil koordinat scroll terakhir jika ada di memori browser
        const savedScrollY = sessionStorage.getItem('blog_scroll_pos');
        if (savedScrollY) {
          // Memberikan jeda mikro agar elemen kartu selesai dirender sepenuhnya
          setTimeout(() => {
            window.scrollTo(0, parseInt(savedScrollY, 10));
            // Bersihkan memori agar posisi reset ke atas jika halaman direfresh manual
  ...
  // Fungsi mencatat posisi koordinat Y tepat sebelum berpindah rute
  const handleCardClick = () => {
    sessionStorage.setItem('blog_scroll_pos', window.scrollY);
  };
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="border-b border-slate-100 dark:border-white/5 pb-8 mb-16">
          <h4 className="text-red-600 font-mono text-[10px] tracking-[0.5em] uppercase font-bold mb-3 italic">
            // Connected_Data_Source: Headless_WordPress
          </h4>
              ...
              return (
                ...
                >
                  ...
                  <div className="pt-4 border-t border-slate-200/40 dark:border-white/5">
                    {/* PASANG EVENT onClick UNTUK MENGEKSEKUSI PENCATATAN KOORDINAT */}
                    <Link 
                      to={`/blog/${post.slug}`} 
